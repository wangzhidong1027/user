  ...
  methods: {
    ...
    getcode() {
      ...
      if (this.time != 60) {
        ...
        this.$axios
          .post(
            this.$baseUrl + "/toolcate/sendSmsLogin",
            this.$qs.stringify({
    ...
    getinfo() {
      this.$axios
        .post(this.$baseUrl + "/per/getuser", this.$qs.stringify({}))
        .then(result => {
          var res = JSON.parse(this.$base64.decode(result.data));
          if (res.code === 10000) {
            this.$vux.toast.show({
    ...
    login() {
      if (this.$refs.phone.valid && this.$refs.code.valid) {
        let formdata = this.$base64.encode(
          JSON.stringify({ mobile: this.phone, code: this.code })
        );
